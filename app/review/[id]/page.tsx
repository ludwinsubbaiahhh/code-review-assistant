...
export default async function ReviewDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!report) return <div>Report data is corrupted.</div>;

  const totalIssues = report.detailedAnalysis?.reduce((acc, section) => acc + (section.issues?.length || 0), 0) || 0;

  return (
    <div className="space-y-6">
      <Link href="/dashboard" className="flex items-center gap-2 text-sm text-slate-600 hover:text-slate-900"><ArrowLeft className="h-4 w-4" />Back to Dashboard</Link>
      <div className="flex items-baseline gap-4"><FileText className="h-8 w-8 text-purple-600" /><div><h1 className="text-3xl font-bold">{review.fileName}</h1><p className="text-slate-500">Analysis performed on {new Date(review.createdAt).toLocaleString()}</p></div></div>
